...
import { loveNoteConfig, userDetails } from "@/config/valentineConfig";
...
const PageLoveNote = ({ onNext }: PageLoveNoteProps) => {
  ...
  const { paragraphs } = loveNoteConfig;
  ...
  useEffect(() => {
    audioRef.current = new Audio(loveNoteConfig.typewriterSoundUrl);
  ...
  const handleParagraphComplete = (index: number) => {
    ...
    if (index < paragraphs.length - 1) {
      setTimeout(() => { setCurrentParagraph(index + 1); }, 400);
    }
  };

  return (
    <div className="min-h-screen flex flex-col items-center px-4 py-8 relative">
      <motion.div animate={{ y: [0, -10, 0], rotate: [0, 10, 0] }} transition={{ duration: 3, repeat: Infinity }} className="absolute top-20 left-10 text-3xl opacity-30">💌</motion.div>
      <motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ duration: 2, repeat: Infinity }} className="absolute top-28 right-12 text-2xl opacity-25">✨</motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-6">
        <motion.p initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="font-script text-xl md:text-2xl text-muted-foreground mb-2">
          {loveNoteConfig.subtitle}
        </motion.p>
        <motion.h1 initial={{ scale: 0.9 }} animate={{ scale: 1 }} transition={{ delay: 0.3, type: "spring" }} className="valentine-title">
          {loveNoteConfig.title}
        </motion.h1>
      </motion.div>
      ...
      >
        <motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ duration: 2, repeat: Infinity }} className="absolute top-2 left-2 text-primary/20 text-xl">♡</motion.div>
        <motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ duration: 2, repeat: Infinity, delay: 0.5 }} className="absolute top-2 right-2 text-primary/20 text-xl">♡</motion.div>

        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }} className="flex items-center justify-between mb-6">
          <motion.span animate={{ y: [0, -8, 0], x: [0, 3, 0] }} transition={{ duration: 2.5, repeat: Infinity }} className="text-4xl">🐝</motion.span>
          <div className="flex items-center gap-3">
            <motion.span animate={{ scale: [1, 1.3, 1] }} transition={{ duration: 1.5, repeat: Infinity }} className="text-2xl">❤️</motion.span>
            <span className="font-script text-2xl text-foreground">{userDetails.greeting}</span>
          </div>
          <motion.img
            initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.8, type: "spring" }}
            whileHover={{ scale: 1.1, rotate: 5 }}
            src={loveNoteConfig.profileImage}
            alt="Profile"
        ...
        <motion.div
          ...
          animate={{ minHeight: `${60 + (completedParagraphs.length + 1) * 80}px` }}
        ...
        <div className="flex justify-center gap-2 mb-4">
          {paragraphs.map((_, index) => (
            <motion.div
              key={index}
              initial={{ scale: 0 }}
              animate={{ scale: 1, backgroundColor: completedParagraphs.includes(index) ? "hsl(var(--primary))" : index === currentParagraph ? "hsl(var(--primary) / 0.5)" : "hsl(var(--muted))" }}
      ...
      <motion.button
        ...
        animate={{ opacity: completedParagraphs.length === paragraphs.length ? 1 : 0.5, y: 0 }}
